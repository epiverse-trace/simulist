...
.create_linelist <- function(scenario) {
  ...
  linelist <- data.frame(
    ...
    time_rounded = c(
      0, 1, 1, 2, 1, 1, 3, 2, 2, 2, 5, 2, 3, 2, 3, 3, 3, 7, 4, 3, 9, 4, 3, 3,
      3, 10, 4, 5, 4, 5, 5, 5, 6, 7, 8, 7, 9, 10, 8, 9, 10, 10
    ),
  ...
  if (scenario >= 4) {
    linelist <- cbind(
      linelist,
      hospitalisation = c(
        8.522327, 5.829065, 5.264541, 5.477855, 2.627005, 1.427216, 13.785723,
        2.166423, 6.261054, 2.395381, 19.198035, 2.176427, 5.113270, 6.463385,
        3.035459, 7.920272, 9.335563, 7.287531, 3.631773, 4.680531, 15.605724,
        4.385845, 5.883632, 7.683852, 3.272805, 20.281291, 4.086057, 5.837060,
        5.478222, 8.518551, 6.191753, 5.602890, 9.145817, 15.788956, 9.358647,
        7.306673, 14.834163, 10.131187, 12.115475, 21.929356, 12.193475,
        10.074229
      ),
      hosp_rounded = c(
        NA, NA, NA, 5, NA, 1, 14, NA, 6, 2, NA, NA, NA, NA, 3, 8, 9, NA, NA, NA,
        NA, 4, NA, 8, 3, 20, 4, NA, NA, 9, 6, NA, 9, 16, 9, 7, NA, NA, 12, NA,
        12, NA
      )
    )
  }

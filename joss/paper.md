...
![A connected dot plot of the timeline of events in a simulated line list.\label{fig:events}](multi-dot-plot.png)

![The daily incidence of symptom onset, hospital admission and deaths facetted by sex for a simulated line list and aggregated and plotted with `incidence2` [@taylorIncidence2ComputeHandle2020].\label{fig:incidence}](incidence.png) 

![A simulated contact network (line list and contact tracing dataset), converted to an `<epicontacts>` object and plotted with `epicontacts` (this network is interactive when rendered in an IDE or online).\label{fig:network}](network.png)

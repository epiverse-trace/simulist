...
Instead of plotting the number of cases on a particular day, we can plot each individual's events over the course of their infection.

We start by simulating a line list. We include an onset-to-recovery delay distribution so all cases have an outcome, a reporting delay so the date cases are reported is after their symptom onset, and set a high risk of hospitalisation to get many hospital admission events. We subset to the first 10 cases so the events on the plot are clear, however, this subsetting is not required.

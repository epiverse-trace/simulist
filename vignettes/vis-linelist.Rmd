...
## Visualising individual event dates

Instead of plotting the aggregated number of cases, deaths, or other events, we can plot each individuals timeline of events.

For this example we will use a reporting delay distribution so the date of reporting lags after the date of symptom onset. We also supply an onset-to-recovery delay distribution so that all outcomes (died or recovered) have an event date. By default outcome dates for cases that recover are missing.

```{r, define-reporting-and-recovery-delays}
...
We can now simulate a new line list with dates for symptom onset, reporting, hospital admission and outcome for each case.
...
```{r, plot-case-events}
#| fig.alt: >
#|   Case timelines showing key epidemiological events for individual cases.
#|   Each horizontal line represents one case, ordered by date of onset. Points
#|   mark four event types recorded in the line list: date of onset (blue
#|   circles), date of admission (red squares), date of reporting (purple
#|   diamonds), and date of outcome (green triangles). The temporal spacing of
#|   these markers illustrates variation in delays between symptom onset,
#|   healthcare seeking, reporting, and eventual outcomes across cases.
...
```{r, subset-linelist-tidyverse, echo=c(1, 3)}
library(dplyr)
# nolint start: one_call_pipe_linter.

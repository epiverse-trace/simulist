...
The reporting delay can be specified as a (random) number geneating function to the `config` argument in `sim_linelist()`. The function can be defined with the name `reporting_delay` to `create_config()`.

```{r config-variable-reporting-delay}
config <- create_config(reporting_delay = function(x) rlnorm(n = x, meanlog = 1, sdlog = 1))
```
...
If instead of a variable reporting delay you want to simulate a fixed reporting delay, for example, the date of reporting is always 5 days after the date of symptom onset, then a different function can be supplied to `create_config(reporting_delay = ...)`. One option is to set the parameter that controls the variance of the probability distribution to zero (in the case of the lognormal distribution, setting `sdlog = 0`), but a simpler and more readable option would be to give a number generating function that always returned 5. 

```{r config-fixed-reporting-delay}
config <- create_config(reporting_delay = function(x) rep(5, times = x))
```
...
trunc_point <- data.frame(
  ...
  trunc_point = as.Date(
    max(
      unlist(linelist[grep(pattern = "date_", x = colnames(linelist))]),
      na.rm = TRUE
...
[^1]: `sim_linelist()` simulates an outbreak to extinction exept in cases where the number of infected individuals exceeds the maximum outbreak size specified in the `outbreak_size` argument in `sim_linelist()`, which by default is `r max(eval(formals(sim_linelist)$outbreak_size))` where the line list returned is an ongoing outbreak.

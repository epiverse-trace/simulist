...
If the age-stratified risk table does not match the default (`c(0, 90)`), the `population_age` argument will need to be set to match.

:::

For example, the default age range of the population is 0 to 90 (inclusive). In our example above, the lowest age group started at 0 and the oldest age group stopped at 90. This matches the default `population_age = c(0, 90)`. However, see here that if the lower age limit exceeds the age range the function will not run.

...
The main function in the {simulist} package is `sim_linelist()`. This functions takes in arguments that control the dynamics of the outbreak, such as the serial interval distribution, and outputs a table (`<data.frame>`) with case information for each infected individual.

For this example we will simulate a linelist for a COVID-19 (SARS-CoV-2) outbreak.
...
The reproduction number is supplied, here we set it as 1.1:
...
When requiring a linelist that represents a large outbreak, such as the COVID-19 outbreak, setting the `min_chain_size` to a larger number guarantees a linelist of that size. Here we simulate a linelist requiring at least 250 cases.
